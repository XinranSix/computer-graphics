...
#pragma region shader
    auto shader { std::make_shared<Shader>("asset/shader/learnopengl/73.geometry_shader_normals/73.default.vs",
                                           "asset/shader/learnopengl/73.geometry_shader_normals/73.default.fs") };
    auto normalDisplayShader { std::make_shared<Shader>(
        "asset/shader/learnopengl/73.geometry_shader_normals/73.normal_visualization.vs",
        "asset/shader/learnopengl/73.geometry_shader_normals/73.normal_visualization.fs") };
    ...
    try {
        // open files
        gShaderFile.open("asset/shader/learnopengl/73.geometry_shader_normals/73.normal_visualization.gs");
    ...
    glAttachShader(normalDisplayShader->ID, geometryShader);
    glLinkProgram(normalDisplayShader->ID);
    glGetProgramiv(normalDisplayShader->ID, GL_LINK_STATUS, &success);
    if (!success) {
        glGetProgramInfoLog(normalDisplayShader->ID, 1024, NULL, infoLog);
    ...
    std::vector<std::shared_ptr<Shader>> shaders { shader, normalDisplayShader };
#pragma endregion
