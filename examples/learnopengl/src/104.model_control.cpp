...
int main() {
    ...
    // -------------------------
    Shader shader("asset/shader/learnopengl/104.model_control/104.shader.vs",
                  "asset/shader/learnopengl/104.model_control/104.shader.fs");
    ...
    // load textures
    // -------------
    unsigned int floorTexture = loadTexture("asset/texture/rock.jpg");

    Model Zhang3 = Model("asset/model/backpack/backpack.obj");
    _Models["zhang3"] = ModelInfo { &Zhang3, glm::vec3(-3, 0, 0), 0.0, 0.0, 0.0, false, "Zhang3" };
    Model Li4 = Model("asset/model/backpack/backpack.obj");
    ...
    while (!glfwWindowShouldClose(window)) {
        ...
        // model
        for (map<string, ModelInfo>::iterator _modelsIter = _Models.begin(); _modelsIter != _Models.end();
             _modelsIter++) {
            model = glm::mat4(1.0f);
...
void mouseClick_callback(GLFWwindow *window, int button, int action, int mods) {
    ...
    if ((action == GLFW_PRESS) && (button == GLFW_MOUSE_BUTTON_LEFT)) {
        lastX = (float)winX;
        lastY = (float)winY;
        LeftButtonHolding = true; // Ö»ÓÐ°´ÏÂÊó±êÓÒ¼üÊ±£¬²Å»á¸Ä±äÉãÏñ»ú½Ç¶È
    } else
    ...
    if ((action == GLFW_PRESS) && (button == GLFW_MOUSE_BUTTON_RIGHT)) {
        ...
        for (map<string, ModelInfo>::iterator _modelsIter = _Models.begin(); _modelsIter != _Models.end();
            ...
            if (_distance < 2.5) {
                cout << _modelsIter->first << "Ä£ÐÍ±»Ñ¡ÖÐ..." << endl;
...
glm::vec3 worldPosFromViewPort(int winX, int winY) {
    float winZ;
    glReadPixels(winX, (int)SCR_HEIGHT - winY, 1, 1, GL_DEPTH_COMPONENT, GL_FLOAT, &winZ);
