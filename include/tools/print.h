...
namespace _print_details {
    ...
    struct _enable_if_map<
        T, U,
        std::enable_if_t<
            std::is_same_v<typename T::value_type, std::pair<typename T::key_type const, typename T::mapped_type>>>> {
        using type = U;
    };
    ...
    /* template <class T, class U> */
    /* struct _enable_if_iterable<T, U, std::void_t<typename std::iterator_traits<typename
    ...
    struct _printer<T,
        ...
        static void print(T const &t) {
            _unrolled_print(
                t, std::make_index_sequence<std::max(static_cast<std::size_t>(1), std::tuple_size_v<T>) - 1> {});
        }
    };
    ...
    struct _printer<T, typename _enable_if_has_print<T, typename _enable_if_variant<T>::type>::not_type> {
        static void print(T const &t) {
            std::visit([](auto const &v) { _printer<_rmcvref_t<decltype(v)>>::print(v); }, t);
        }
    };
    ...
    public:
        ...
        friend std::ostream &operator<<(std::ostream &os, print_adaptor const &&self) {
            ...
            os << "[object 0x" << std::hex
               << reinterpret_cast<uintptr_t>(reinterpret_cast<void const volatile *>(std::addressof(self.t))) << ']';
            os.flags(oldflags);
            return os;
        }
    };
