...
namespace ht {
    ...
    Mesh Model::processMesh(aiMesh *mesh, const aiScene *scene) {
        ...
        for (unsigned int i = 0; i < mesh->mNumVertices; i++) {
                                         ...
                                         // coordinates?
            {
                ...
                // coordinates. We thus make the assumption that we won't
        ...
        // a sequential number ranging from 1 to MAX_SAMPLER_NUMBER. Same
        // applies to other texture as the following list summarizes:
    ...
    vector<Texture> Model::loadMaterialTextures(aiMaterial *mat,
        ...
        for (unsigned int i = 0; i < mat->GetTextureCount(type); i++) {
            ...
            // next iteration: skip loading a new texture
            bool skip = false;
            for (unsigned int j = 0; j < textures_loaded.size(); j++) {
                if (std::strcmp(textures_loaded[j].path.data(), str.C_Str()) ==
                    ...
                    skip = true; // a texture with the same filepath has
                                 // already been loaded, continue to next
                                 // one. (optimization)
                    break;
                }
            }
            if (!skip) { // if texture hasn't been loaded already, load it
                ...
                textures_loaded.push_back(
                    texture); // store it as texture loaded for entire
                              // model, to ensure we won't unnecessary load
                              // duplicate textures.
            }
        }
        return textures;
