...
float shape1(in vec3 p) {
    float d = sdfSphere(p);
    d = min(d, sdfBox(p - vec3(0.0,1.0, 0.0), vec3(0.4))); // 求并
    return d;
}

float shape2(in vec3 p) {
    float d = sdfSphere(p);
    d = max(d, sdfBox(p - vec3(0.0,1.0, 0.0), vec3(0.4, 2.0, 0.4))); // 求交
    return d;
}

float shape3(in vec3 p) {
    float d = sdfSphere(p);
    d = max(d, -1.0 * sdfBox(p - vec3(0.0,0.0, 0.0), vec3(2.0, 0.4, 0.4))); // 求差
    return d;
}
...
vec2 map(in vec3 p) {
    vec2 d = vec2(shape1(p - vec3(-2.0, 1.0, 0.0)), 2.0);
    d = opU(d, vec2(shape2(p - vec3(2.0, 1.0, 0.0)), 3.0));
    d = opU(d, vec2(shape3(p - vec3(0.0, 1.0, 2.0)), 4.0));
